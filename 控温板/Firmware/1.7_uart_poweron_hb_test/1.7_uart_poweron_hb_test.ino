...
#define UART_POWERON_HB_TEST       true
#define UART_POWERON_HB_INTERVAL_MS 100UL
...
#if UART_POWERON_HB_TEST
static uint32_t uartPoweronHbLastMs;
...
static void SerialReportWriteStringBlocking(const char *text) {
  while (*text) {
    SerialReportWriteBlocking(*text++);
  }
}

static void UartPoweronHbTask() {
  uint32_t now = millis();
  if ((now - uartPoweronHbLastMs) < UART_POWERON_HB_INTERVAL_MS) return;
  uartPoweronHbLastMs = now;
  SerialReportWriteStringBlocking("HB\n");
}
#endif
...
void setup() {
  ...
  SerialReportInit();
#if UART_POWERON_HB_TEST
  SerialReportWriteStringBlocking("BOOT\n");
  SerialReportWriteStringBlocking("HB\n");
#endif
#endif
...
void loop() {
#if SERIAL_REPORT_ENABLE && UART_POWERON_HB_TEST
  UartPoweronHbTask();
  return;
#endif
