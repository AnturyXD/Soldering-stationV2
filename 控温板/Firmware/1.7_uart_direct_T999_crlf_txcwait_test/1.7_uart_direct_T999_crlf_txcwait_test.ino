...
#define UART_AB_TEST_DISABLE_OLED  true
#define UART_FIXED_FRAME_RUNTIME_TEST true
...
#if UART_FIXED_FRAME_RUNTIME_TEST
static uint32_t fixedFrameLastMs;

static void SerialReportWriteBlocking(char c) {
  while (!(UCSR0A & _BV(UDRE0))) {
  }
  UDR0 = c;
}

static void SerialReportWaitComplete() {
  while (!(UCSR0A & _BV(TXC0))) {
  }
  UCSR0A |= _BV(TXC0);
}

static void SerialReportWriteTextBlocking(const char *text) {
  while (*text) {
    SerialReportWriteBlocking(*text++);
  }
}

static void FixedFrameRuntimeTask() {
  uint32_t now = millis();
  if ((now - fixedFrameLastMs) < SERIAL_REPORT_INTERVAL_MS) return;
  fixedFrameLastMs = now;
  UCSR0A |= _BV(TXC0);
  SerialReportWriteBlocking('$');
  SerialReportWriteBlocking('T');
  SerialReportWriteBlocking(',');
  SerialReportWriteBlocking('9');
  SerialReportWriteBlocking('9');
  SerialReportWriteBlocking('9');
  SerialReportWriteBlocking(',');
  SerialReportWriteBlocking('E');
  SerialReportWriteBlocking('\r');
  SerialReportWriteBlocking('\n');
  SerialReportWaitComplete();
}
#endif
...
#if SERIAL_REPORT_ENABLE
#if UART_FIXED_FRAME_RUNTIME_TEST
  FixedFrameRuntimeTask(); // runtime logic is active, but UART payload is fixed
#else
...
void resetEEPROMDefaults() {
  ...
  for (uint8_t i = 0; i < TIPMAX; i++) {
    CalTemp[i][0] = TEMP200;
    CalTemp[i][1] = TEMP280;
    CalTemp[i][2] = TEMP360;
    CalTemp[i][3] = TEMPCHP;
    for (uint8_t j = 0; j < TIPNAMELENGTH; j++) TipName[i][j] = 0;
  }

  const char defaultTipName[] = TIPNAME;
  for (uint8_t j = 0; j < (TIPNAMELENGTH - 1) && defaultTipName[j]; j++) {
    TipName[0][j] = defaultTipName[j];
  }
...
void getEEPROM() {
  ...
  if (identifier == EEPROM_IDENT) {
    ...
    if (NumberOfTips < 1 || NumberOfTips > TIPMAX ||
        CurrentTip >= NumberOfTips ||
        MainScrType > 1 || PIDenable > 1 || beepEnable > 1 ||
        LANG > 2 || FlipState > 1) {
      resetEEPROMDefaults();
      return;
    }
...
void updateEEPROM() {
  ...
  if (NumberOfTips > TIPMAX) NumberOfTips = TIPMAX;
