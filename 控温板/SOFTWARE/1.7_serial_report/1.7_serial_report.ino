...
bool lastbutton;
#include "Arduboy2.h"  // Local trimmed graphics-only compatibility layer for ATmega328P
Arduboy2 arduboy;
...
#define SERIAL_REPORT_INTERVAL 1000UL
...
void loop() {
  ...
  reportStatus();     // send short telemetry for the external controller
  //beep(0);
}
...
void reportStatus() {
  ...
  uint32_t now = millis();
  if (now - lastReport < SERIAL_REPORT_INTERVAL) return;
  lastReport = now;

  Serial.print(F("T:"));
  Serial.print((uint16_t)ShowTemp);
  Serial.print(F(",S:"));
  Serial.print((uint16_t)SetTemp);
  Serial.print(F(",V:"));
  Serial.println((uint16_t)Vin);
}
